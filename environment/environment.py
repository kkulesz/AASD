...
class Environment:
    def __init__(self, config: EnvironmentConfig, logger: Logger):
        ...
        self.trucks = [
            TruckAgent(truck_jid1, consts.COMMON_PASSWORD, consts.SUPERVISOR_JIT, 3, self.logger, TruckLogic(self.logger, Cords(0, 0), {config.landfills_data[0].jid: Cords(config.landfills_data[0].position.x, config.landfills_data[0].position.y)}, max_volume=110)),
            TruckAgent(truck_jid2, consts.COMMON_PASSWORD, consts.SUPERVISOR_JIT, 3, self.logger, TruckLogic(self.logger, Cords(0, 0), {config.landfills_data[0].jid: Cords(config.landfills_data[0].position.x, config.landfills_data[0].position.y)}, max_volume=110))
        ]
